...
contract MachoDoge is Context, IERC20, Ownable {
    ...
    string private _name = 'MachoDoge';
    ...
    uint256 private _maxTxAmount = 5 * 10**9 * 10**9;
    // We will set a minimum amount of tokens to be swaped => 50000
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (!inSwap && swapEnabled && overMinTokenBalance && sender != uniswapV2Pair) {
            ...
            if(contractETHBalance > 0) {
                sendETHToCharity(contractETHBalance.mul(4).div(7));
                sendETHToMarketing(contractETHBalance.mul(3).div(7));
            }
        }
    ...
    function _setTaxFee(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 0 && taxFee <= 3, 'taxFee should be in 0 - 3');
        _taxFee = taxFee;
    }

    function _setCharityFee(uint256 charityFee) external onlyOwner() {
        require(charityFee >= 0 && charityFee <= 7, 'charityFee should be in 0 - 7');
